/**
 * GET /api/contours — Data Production Phase 5
 *
 * Returns SRTM-derived contour LineString features for a region.
...
import { getContourFeatures, ServiceError } from '@/lib/server/geosignal-service'
...
export async function GET(request: NextRequest) {
  ...
  try {
    const contours = await getContourFeatures(region_id as RegionId, kecamatan_id)
    return NextResponse.json(contours, { status: 200 })
  } catch (err) {
    if (err instanceof ServiceError) {
      return NextResponse.json(
        makeError(ERR.SERVICE_ERROR, 'Failed to retrieve contour features.'),
        { status: 500 },
      )
    }
    return NextResponse.json(
